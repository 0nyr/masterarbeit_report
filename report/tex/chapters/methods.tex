...
Printing Chunk [addr:0xa09fd2438355] [status:free] [footer:correct] Chunk(block_index=8180, size=32, flags=[A=False, M=False, P=True])
    ...
    Since we have seen that free chunks are never annotated, we can filter them out. This filtering technique allows to reduce the number of chunks from $37682063$ to $37346373$, which is a small reduction of 0.89\%. This is not a huge reduction of chunks, but is a much more significant reduction of the number of blocks since 27.2\% of the blocks are in free chunks.

    We can also filter the chunks whose size is not 32, 48 or 64 bytes. This is based on the observation that the key chunks are of those sizes. This filtering technique allows to reduce the number of chunks significantly, since there is a cumulated $109346+34366+13434 = 157146$ chunks of those size, which represents, compare to the original number of chunks, a diminution of 99.6\%. This is indeed a huge reduction of the scale of the problem.ch

    A last approach to chunk filtering for key prediction consists in measuring the entropy of the first bytes of a chunk. Since we have previously discovered that keys are always located at the beggining of a chunk, and since the keys are composed of random bytes, we can expect the entropy of the first bytes of a chunk to be high.
...
At this step, the graph is looking similar to the example shown before \ref{methods:mem_graph:17016-1643962152:simplified}. Using the pointer reduction to a chunk-only memgraph, we can obtain something like the following:
...
Note how we can identify some data structures formed by pointer-connected chunks. This is a very interesting property of the memory graph representation, since it allows to identify data structures and their members based only on the shape of connections. This is a very important property that will be used later for feature engineering and embeddings.

\subsubsection{Custom graph-based embeddings}
4. Generate embedding from graph
different embedding are possible. We present some of those custom embeddings like the semantic embedding which is a general way to embed the graph by addind the related graph structure and vicinity information to each node. Other embeddings are possible, like the statistical embedding, which is based on the statistical properties of the graph, or the random walk embedding, which is based on the random walk algorithm. 
...
Below is an example of a memgraph comment field containing a JSON serialized object with embedding type and feature names.
...
As often with models, Random Forst has a bunch of hyperparameters:
...
We have used the implementation of Random Forest available in the scikit-learn library \cite{scikit-learn}.
...
We have relied on the Logistic Regression implementation available in the scikit-learn library for our experiments\cite{scikit-learn}, using the default hyperparameters.
...
For the SGD Classifier as well, we used the scikit-learn implementation\cite{scikit-learn}.
...
Batch normalization, on the other hand, aims to accelerate training and stabilize the learning process. By normalizing the output features of each layer, batch normalization helps in alleviating internal covariate shift, making the model training more resilient to the choice of hyperparameters.
