...
\subsection{GCN}
* we have already discussed the GCN in the background section, so we can focus on the implementation and the different models used.
* GCN are used for the key chunk prediction task.
* special type of neural network that can operate on graphs: present its main characteristics
* they capture the structure of the graph as part of their input  (using edge connectivity from adjacency matrix or edge list)

\subsubsection{Simple GCN models}
* first models used, with a simple architecture

\subsection{The impact of complexity}
* discuss how the complexity of the model impacts the performance, especially the precision and recall, 
* impact of different numbers on input graphs (few, a lot)
