...
\end{lstlisting}
    ...
    Base on this algorithm, and in a similar fashion as what we have done manually by exploring the heap dump file with vim, we can propose the following algorithm to detect the malloc headers in a heap dump file:
        ...
        \Procedure{MallocHeaderDetection}{$heapDumpFile$}
            ...
            \While{$position < \text{FileSize}(heapDumpFile)$}
                ...
                \If{$\text{block} \neq 0$}
                    ...
                    \State $position \gets position + 8 + size$ \Comment{Leap over data structure. + 8 for the header.}
                \Else
...
$ python src/data_structure_detection.py --input /home/onyr/code/phdtrack/phdtrack_data_clean/Training/Training/basic/V_7_1_P1/24/17016-1643962152-heap.raw --debug
    ...
    The malloc header chaining algorithm allows to detect potential data structures in the heap dump file. However, it does not allow to distinguish between free and allocated chunks. This is a problem, since we want to be able to distinguish between free and allocated chunks, to be able to detect potential data structures. Indeed, we can expect to find some free chunks in the heap dump file, and we don't want to consider them as potential data structures. As such, we need to be able to distinguish between free and allocated chunks.

    % TODO: complete
    Speak about the tests I did with detecting pointers, and how it was not conclusive. I think that the best way to do it is to rely on the P flag of the malloc header. If it is set to 1, it means that the previous chunk is allocated. If it is set to 0, it means that the chunk is free. This is the best way to do it, since we don't have access to the bins in the heap dump file, and since pointers can be anywhere in the heap dump file. 

    

    To do so,

    % TODO: add subsubsection about stats on chunk parsing

    % TODO: About footer, I discovered that footers are actually the size WITH flags
